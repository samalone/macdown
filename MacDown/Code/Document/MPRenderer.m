...
- (NSArray *)baseStylesheets
{
    ...
    // page breaks for printing and PDF export (macdown-ppi.1). The bundle URL
    // is constant for the process lifetime, so resolve it once (baseStylesheets
    // runs on every render).
    static NSURL *printStyle = nil;
    static dispatch_once_t printStyleToken;
    dispatch_once(&printStyleToken, ^{
        printStyle = [[NSBundle mainBundle] URLForResource:@"print"
                                             withExtension:@"css"];
    });
    if (printStyle)
...
- (NSArray *)mathjaxScripts
{
    ...
    // old bundle shipped a patched MathJax.js (for MathJax issue #548) that the
    // legacy WebResourceLoadDelegate swapped in; the stock CDN loader (2.7.3)
    // renders correctly and fires the StartupHook('End') the preview's
    // completion handler relies on, so that patch is no longer needed. Bundling
    // the full MathJax for offline use remains possible future work.
